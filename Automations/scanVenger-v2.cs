...
class Program
{
    ...
    static async Task<string> ResolveHostname(string ip)
    {
        try
        {
            // First, attempt DNS resolution in the current domain
            var hostEntry = await Dns.GetHostEntryAsync(ip);
            return hostEntry.HostName;
        }
        catch
        {
            // Fallback to querying Active Directory
            try
            {
                return GetHostnameFromActiveDirectory(ip);
            }
            catch
            {
                // If all fails, return the IP itself
                return null;  // Return null instead of IP to avoid unnecessary "No match" output
            }
        }
    }


    // Method to get hostname from Active Directory
    // Method to get hostname from Active Directory
    static string GetHostnameFromActiveDirectory(string ip)
    {
        try
        {
            // Access the current domain
            Domain currentDomain = Domain.GetCurrentDomain();
            using (DirectorySearcher searcher = new DirectorySearcher(new DirectoryEntry($"LDAP://{currentDomain.Name}")))
            {
                ...
                foreach (SearchResult result in searcher.FindAll())
                {
                    ...
                    {
                        string dnsHostName = result.Properties["dNSHostName"][0].ToString();
                        Console.WriteLine($"Checking {ip}: Found DNS Hostname {dnsHostName}");

                        if (dnsHostName == ip || dnsHostName.StartsWith(ip))
                            return dnsHostName;
                    }
                }
            }

            // Optionally, extend the search to trusted domains (optional)
            foreach (TrustRelationshipInformation trust in currentDomain.GetAllTrustRelationships())
            {
                Domain trustedDomain = Domain.GetDomain(new DirectoryContext(DirectoryContextType.Domain, trust.TargetName));
                using (DirectorySearcher trustedSearcher = new DirectorySearcher(new DirectoryEntry($"LDAP://{trustedDomain.Name}")))
                {
                    trustedSearcher.Filter = $"(&(objectClass=computer)(name=*))";
                    trustedSearcher.PropertiesToLoad.Add("dNSHostName");

                    foreach (SearchResult result in trustedSearcher.FindAll())
                    {
                        if (result.Properties["dNSHostName"].Count > 0)
                        {
                            string dnsHostName = result.Properties["dNSHostName"][0].ToString();
            ...
            Console.WriteLine($"No match found for {ip} in AD.");
            return ip; // Return IP if no match is found
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Active Directory lookup error for {ip}: {ex.Message}");
            return ip; // Return IP if AD lookup fails
        }
    }
...
╚══════╝ ╚═════╝╚═╝  ╚═╝╚═╝  ╚═══╝  ╚═══╝  ╚══════╝╚═╝  ╚═══╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝
    ...
    static async Task ScanHostForHostDiscovery(string host, List<(string, List<int>, string, string)> results)
    {
        ...
        if (os != "Host Unreachable")
        {
            ...
            // Log the host discovery only if a valid hostname or OS was found
            ...
            {
                // Log the IP itself if no valid hostname
                results.Add((host, new List<int>(), host, os));
            }
        }
    }
